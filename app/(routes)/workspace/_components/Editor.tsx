...
function Editor({onSaveTrigger,fileId,fileData}:{onSaveTrigger:any,fileId:any,fileData:FILE}) {
    ...
    useEffect(()=>{
        if (fileData) {
          ...
          // If user is not logged in or not the creator of the file
          setIsReadOnly(!user || (fileData.createdBt !== user?.email));
          initEditor();
        }
    },[fileData, user])
    ...
    useEffect(()=>{
      ...
      if (onSaveTrigger && !isReadOnly) {
        onSaveDocument();
      }
    },[onSaveTrigger, isReadOnly])
